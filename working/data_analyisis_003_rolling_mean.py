...
for slice_index in range(0, 10):
    for window_scale in range(1, 51):
        ...
        feature_vector = feature_df["signal_mean"]
        feature_df.dropna(inplace=True)
        corr_coeff_ = numpy.corrcoef(feature_df["signal_mean"], feature_df["open_channels"])[0][1]
...
mean_correlation_df.to_csv("mean_correlation.csv", index=False)

...
for slice_index in range(0, 10):
    ...
    for recurrent_id in range(1, 51):
        # print(feature_df)
        ...
        feature_df_temp = feature_df[["signal_recurrent_mean", "open_channels"]].copy()
        feature_df_temp.dropna(inplace=True)
        corr_coeff_ = numpy.corrcoef(feature_df_temp["signal_recurrent_mean"], feature_df_temp["open_channels"])[0][1]
...
mean_correlation_df.to_csv("signal_recurrent_mean_correlation.csv", index=False)
